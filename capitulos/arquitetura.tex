...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    entity/.style={rectangle, draw, fill=blue!10, text width=3cm, text centered, minimum height=1.5cm},
    attribute/.style={ellipse, draw, fill=yellow!10, text width=2cm, text centered, minimum height=0.8cm},
    relationship/.style={diamond, draw, fill=green!10, text width=2cm, text centered, minimum height=1cm},
    isa/.style={triangle, draw, fill=red!10, text width=1.5cm, text centered, minimum height=1cm}
]

% Entidades principais
\node[entity] (empresa) at (0,6) {\textbf{Empresa}\\id, nome, cnpj\\email, latitude\\longitude, raio};
\node[entity] (usuario) at (6,6) {\textbf{Usuario}\\id, nome, email\\password, papel\\status, cpf};
\node[entity] (departamento) at (0,3) {\textbf{Departamento}\\id, nome\\descricao};
\node[entity] (cargo) at (0,0) {\textbf{Cargo}\\id, nome\\descricao\\base\_salarial};
\node[entity] (info-trab) at (6,3) {\textbf{InformacoesTrabalhistas}\\id, data\_admissao\\carga\_horaria\\salario};
\node[entity] (horario-func) at (12,6) {\textbf{HorarioFuncionario}\\id, dia\_semana\\horario\_inicio\\horario\_fim};
\node[entity] (registro-ponto) at (6,0) {\textbf{RegistroPonto}\\id, tipo, status\\data\_hora, latitude\\longitude};
\node[entity] (justificativa) at (12,0) {\textbf{Justificativa}\\id, motivo, tipo\\status, data\_aprovacao};

% Relacionamentos
\draw[thick] (empresa) -- (usuario) node[midway, above] {1:N};
\draw[thick] (empresa) -- (departamento) node[midway, left] {1:N};
\draw[thick] (departamento) -- (cargo) node[midway, left] {1:N};
\draw[thick] (usuario) -- (info-trab) node[midway, right] {1:1};
\draw[thick] (info-trab) -- (cargo) node[midway, below] {N:1};
\draw[thick] (info-trab) -- (departamento) node[midway, above] {N:1};
\draw[thick] (usuario) -- (horario-func) node[midway, above] {1:N};
\draw[thick] (usuario) -- (registro-ponto) node[midway, left] {1:N};
\draw[thick] (registro-ponto) -- (justificativa) node[midway, above] {1:N};

\end{tikzpicture}
}
\caption{Modelo entidade-relacionamento do sistema}
